...
jQuery(document).ready(function($) {
		...
		if ( kopaframework_upload ) {
			...
			kopaframework_upload.on( 'select', function() {
				...
				if ( attachment.attributes.type == 'image' ) {
					kopaframework_selector.find('.kopa_screenshot').empty().hide().removeClass('kopa_hide').append('<img src="' + attachment.attributes.url + '"><a class="kopa_remove_image">' + kopa_admin_l10n.remove + '</a>').slideDown('fast');
				}
				kopaframework_selector.find('.kopa_upload_button').unbind().addClass('kopa_remove_file').removeClass('kopa_upload_button').val(kopa_admin_l10n.remove);
				// kopaframework_selector.find('.of-background-properties').slideDown();
				kopaframework_selector.find('.kopa_remove_image, .kopa_remove_file').on('click', function() {
					kopaframework_remove_file( $(this).closest('.kopa_section') );
				});
			});
	...
	function kopaframework_remove_file(selector) {
		selector.find('.kopa_remove_image').hide();
		selector.find('.kopa_upload').val('');
		// selector.find('.of-background-properties').hide();
		selector.find('.kopa_screenshot').slideUp('fast');
		selector.find('.kopa_remove_file').unbind().addClass('kopa_upload_button').removeClass('kopa_remove_file').val(kopa_admin_l10n.upload);
		// We don't display the upload button if .upload-notice is present
		// This means the user doesn't have the WordPress 3.5 Media Library Support
		if ( $('.kopa_section_upload .kopa_upload_notice').length > 0 ) {
			$('.kopa_upload_button').remove();
		}
		selector.find('.kopa_upload_button').on('click', function(event) {
			kopaframework_add_file(event, $(this).closest('.kopa_section'));
		});
	}

	$('.kopa_remove_image, .kopa_remove_file').on('click', function() {
		kopaframework_remove_file( $(this).closest('.kopa_section') );
    });

    $('.kopa_upload_button').click( function( event ) {
    	kopaframework_add_file(event, $(this).closest('.kopa_section'));
    });
	/* ========================================================================
	 * END >>> Media Uploader
	...
	function add_custom_font(e) {
		...
		var $newItem = $fontList.children('.kopa_custom_font_item:last'),
			...
			// get the jquery objects of the new upload buttons
			$uploadButtons = $newItem.find('.kopa_upload_button');
		...
		// and for its upload file button
